                    ...
                    <div className=' mt-8'>
                        {
                            taskStatus.length>0 ? <> {taskStatus.map(task => <div className='shadow-lg p-4 mb-4 rounded-xl' key={task.id}>
                                <h2 className='text-xl font-bold'>{task.problem_title}</h2>
                                <button onClick={()=>{handleResolve(task)}} className='w-full mt-4 bg-green-600 py-2 rounded-lg text-white text-lg'>Complete</button>
                            </div>) }
